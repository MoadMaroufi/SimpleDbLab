...
public class BufferPool {
    ...
    */
    ...
    // We are using LRU-k as an evitction policy thus we have to keep track of the access history of each page
    //We use a queu that can support k elments, first in first out mechanism
    //
    ...
    private void evictPage() throws DbException {
        //We go over the oldest access time of each page (some pages haven't been yet accesses k times
        // so we take the oldest time in their access history instead) and then we evict the one with the oldest kth access time.
