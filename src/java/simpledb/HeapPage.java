...
public class HeapPage implements Page {
    ...
    */
    private int getNumTuples() {
        int pageSize = BufferPool.getPageSize();//this is a static method i.e always present in the class and common to all instances
        double pageSizeBits = pageSize * 8.0; // Convert to bits
        double tupleSizeBits = this.td.getSize() * 8.0 + 1;
        return (int) Math.floor(pageSizeBits/tupleSizeBits);
    ...
    private int getHeaderSize() {        
        int NumTypPerPage=getNumTuples();
        return (int) Math.ceil(NumTypPerPage / 8.0);
    }
    ...
    public Iterator<Tuple> iterator() {
        ...
        public boolean hasNext() {
            return findNextIndex(currPosition) != -1;
        }

        @Override
        public Tuple next() {
            currPosition = findNextIndex(currPosition);
            if (currPosition == -1) {
                throw new NoSuchElementException();// either or slots are empty or we consumed all of them
            }
            Tuple nextTuple=tuples[currPosition];
            currPosition++; // Move to the next position for subsequent calls
            return  nextTuple;
        }
        @Override
        public void remove() {
            ...
            private int findNextIndex(int startIndex) {
                while (startIndex < numSlots) {
                    if (isSlotUsed(startIndex)) {
                        return startIndex;
                    }
                    startIndex++;
                }
                return -1; // No more used slots
    }
